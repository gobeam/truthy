...
describe('RefreshTokenService', () => {
  ...
  describe('resolveRefreshToken', () => {
    it('test for malformed token', async () => {
      ...
      jest.spyOn(service, 'decodeRefreshToken').mockResolvedValue({ jti: 1, sub: 1 });
  ...
  it('createAccessTokenFromRefreshToken', async () => {
    jest.spyOn(service, 'resolveRefreshToken').mockResolvedValue({ user, token: refreshToken });
    // jest
  ...
  describe('getUserFromRefreshTokenPayload', () => {
    it('check get user from refresh token with malformed token', async () => {
      await expect(service.getUserFromRefreshTokenPayload({ jti: null, sub: null })).rejects.toThrowError(
        CustomHttpException
      );
    ...
    it('get user from valid refresh token', async () => {
      ...
      await expect(service.getUserFromRefreshTokenPayload({ jti: 1, sub: 1 })).resolves.not.toThrow();
  ...
  describe('getStoredTokenFromRefreshTokenPayload', () => {
    it('check for malformed token', async () => {
      await expect(service.getStoredTokenFromRefreshTokenPayload({ jti: null, sub: null })).rejects.toThrowError(
        CustomHttpException
      );
    });
    ...
    it('get stored token from refresh token payload', async () => {
      ...
      await expect(service.getStoredTokenFromRefreshTokenPayload({ jti: 1, sub: 1 })).resolves.not.toThrow();
